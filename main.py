...
def has_already_guessed(candidate):
    if candidate in guesses:
        print(f"You've already guessed the word '{candidate}'. Try again!")
        return True
    return False
...
while not is_game_over():
    ...
    if has_already_guessed(guess):  # Check for duplicates first
        continue
    ...
    if guess in full_list:
        ...
        if guessed == WORDS_TO_WIN:
            congratulate_user()
            exit()
        print(f"That's right! {WORDS_TO_WIN - guessed} more to go")
    else:
        errors += 1
